...
function reset_profile() {
    echo "resetting bash to default (clears personal tweaks)..."
    bash ~/.sauce/scripts/stow.sh restow
    printf '%s\n' "# add your personal bash tweaks here" > ~/.config/sauce/user.sh
    exec bash
}
...
alias bashrc=refresh_profile
alias bashrc-reset=reset_profile
...
if [ -d "$HOME/.config/sauce/profile.d/posix" ]; then
    for _frag in "$HOME/.config/sauce/profile.d/posix"/*.sh; do
