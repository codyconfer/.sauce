...
function reset_profile
    echo "resetting fish to default (clears personal tweaks)..."
    bash ~/.sauce/scripts/stow.sh restow
    printf '%s\n' "# add your personal fish tweaks here" > ~/.config/sauce/user.fish
    exec fish
end
...
alias fishrc 'refresh_profile'
alias fishrc-reset 'reset_profile'
alias sauce-stow 'bash ~/.sauce/scripts/stow.sh'

if test -d ~/.sauce/scripts
    for _script in ~/.sauce/scripts/update-*.sh ~/.sauce/scripts/install-*.sh
        test -e $_script; or continue
        alias (basename $_script .sh) "bash $_script"
    end
end

if test -d "$HOME/.config/sauce/profile.d/fish"
    for _frag in (find "$HOME/.config/sauce/profile.d/fish" -maxdepth 1 -name '*.fish' -type f 2>/dev/null | sort)
        test -r "$_frag"; and source "$_frag"
    end
    set -e _frag
end
...
test -r "$HOME/.config/sauce/user.fish"; and source "$HOME/.config/sauce/user.fish"
